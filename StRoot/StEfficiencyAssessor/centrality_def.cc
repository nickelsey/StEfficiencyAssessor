...
void CentralityDef::setVzParameters(double par0, double par1, double par2,
...
void CentralityDef::setCentralityBounds16Bin(
...
void CentralityDef::setWeightParameters(const std::vector<double> &pars,
                                        double bound) {
  weight_par_.clear();
  weight_bound_ = 0;
  if (pars.size() != 7) {
...
bool CentralityDef::checkEvent(int runid, double refmult, double zdc,
                               double vz) {
  if (refmult < 0)
    return false;
